...
void main() {
  ...
  group('SignInImpl Test', () {
    ...
    test('deve retornar um erro caso os dados estejam invalidos', () async {
      when(repository.signIn(any, any))
          .thenAnswer((_) async => Right(ResponseModel.fromMap(jsonResponse)));
      final result = await usecase.call(null, null);
      expect(result!.fold(id, id), isA<IError>());
    });
    test('deve retornar um erro caso os dados estejam vazios', () async {
      when(repository.signIn(any, any))
          .thenAnswer((_) async => Right(ResponseModel.fromMap(jsonResponse)));
